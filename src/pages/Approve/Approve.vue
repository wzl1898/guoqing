<template>
  <div class="Container">
    ...
    <div class="content">
      <div class="card" v-for="(item, idx) in items" :key="idx" @click="$router.push('/approve-process/' + item.id)">
        <div class="cardHeader">
          <p class="cardHeaderTitle"> {{item.leader}}提交的用证申请 </p>
          <p class="cardHeaderTime"> {{item.createTime}} </p>
        </div>
        ...
        <div class="cardFooter">
          <van-tag size="medium" type="primary">{{item.leader[0]}}</van-tag>
          <p class="footerName"> 由{{item.leader}}提交 </p>
...
<script>
import {getData} from '@/api/approve.js';
export default {
  name: "Approve",
  data(){
    return {
